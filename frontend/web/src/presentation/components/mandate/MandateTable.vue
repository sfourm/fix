...
<template>
  <div class="table-wrap">
    <table class="table">
      ...
      <tbody>
        <tr v-for="m in mandates" :key="m.id" class="clickable" @click="router.push(`/mandates/${m.id}`)">
          ...
          <td>{{ m.terms.tenor ?? '—' }}</td>
          <td>{{ m.terms.quantity === null ? 'sem teto' : `${formatNumber(m.terms.quantity)} ${unitLabel[m.terms.quantityUnit]}` }}</td>
          <td>{{ m.balance === null ? '—' : formatNumber(m.balance) }}</td>
